  ...
  return (
    <div className='w-[70px] h-full fixed left-0 top-0 flex flex-col justify-between border-r border-gray-200 px-4 py-10 z-10'>
      <a href="/#home">
        ...
        <span className='block w-min rotate-90 origin-bottom text-[12px] font-semibold'>Meledath</span>
      </a>
      <div className="flex flex-col gap-y-3 sm:gap-y-2">
        ...
        <a href={`/#${item.id}`} key={i} className='group flex flex-col items-center gap-y-2'>
            ...
            item.id===id?"text-red-500 scale-110" :"text-yellow-500 scale-100"
            }`}>{item.icon}</span>
            <span className={`text-[10px] tracking-wide -translate-x-2 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition duration-300 text-center
                ...
                ${item.id===id && "-translate-x-0 opacity-100" }`}>{item.name}</span>
        </a>
        ))}
      </div>
      <p className='flex items-center justify-center text-[13px] text-gray-500 mt-6'>
        <span className='absolute left-1/2 w-max flex items-center -rotate-90 origin-bottom-left tracking-wider'>{copyRightIcon} 2022-{new Date().getFullYear()}</span>
      </p>
    </div>
  )
}
